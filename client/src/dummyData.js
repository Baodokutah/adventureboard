export const Post = [
    {
      id: 1,
      title: "Hỗ trợ công tác văn phòng lấy số đo cân nặng cho mẹ của bạn",
      tags: ["Sang", "1 ngay", "chieu"],
      content: "[CTXH] TUYỂN 10 CTV HỖ TRỢ VĂN PHÒNG\n1. Nội dung: Lấy số đo cân nặng mẹ của bạn xong làm khảo sát\n2. Thời gian: 8h15 – 11h00 (ca sáng) và 13h15 - 16h00 (ca chiều), Ngày 31/02/2069 (Thứ Nhất).\n3. Địa điểm: Tập trung tại Văn phòng Đoàn (114B1) – Cơ sở Lý Thường Kiệt để được điều phối.\n4. Quyền lợi: 0.5 ngày CTXH\n5. Lưu ý:\n- Ăn mặc lịch sự\n- Đúng giờ.\n- Bắt buộc làm khảo sát nộp về mới tính CTXH.\n- Liên hệ điểm danh: 0969 420 420 (A. Tèo)"
    },
    {
      id: 2,
      title: "Tìm nhóm bài tập lớn môn phương pháp tính CC69 đề tài lấy cân nặng cho mẹ của bạn",
      tags: ["MT1009", "CC"],
      content: "🔥 Cần gấp 3  bạn đồng hành cho nhóm bài tập lớn lớp CC69! Đề tài siêu hài: 'Dùng phương pháp cân trái đất để đo cân nặng cho mẹ của bạn.' \n💡 Tuyển: Bạn nào sáng tạo, thích thách thức, và sẵn lòng đối mặt với một đề tài hài hước nhưng không kém phần 'chất'. \n🎯 Mục tiêu: Điểm A+ và một mùa thi không đuối sức với tiếng cười. \n🤝 Hãy nhanh tay inbox hoặc comment ngay để cùng tạo nên kỳ tích! Fb: https://www.facebook.com/@Bietongthuongko"
    },
    {
      id:3,
      title: "BTL phương pháp tính CN01 thầy Tú Hai Cuẩn",
      tags: ["MT1009", "CN01"],
      content: "🔥 Cần gấp 3 bạn đồng hành cho nhóm bài tập lớn lớp CN01! Đề tài: 'Phương pháp tính toán hiệu quả trong thực tế.' \n💡 Tuyển: Bạn nào có tư duy logic tốt, thích thách thức, và sẵn lòng đối mặt với một đề tài thực tế. \n🎯 Mục tiêu: Điểm A+ và một mùa thi không đuối sức. \n🤝 Hãy nhanh tay inbox hoặc comment ngay để cùng tạo nên kỳ tích! Hoặc liên hệ với mình qua Fb: https://www.facebook.com/@Bietongthuongko"
    }
];
export const Comment = [
    {
      id: 1,
      avatar: "https://media.discordapp.net/attachments/1067775825048510534/1084508310272737420/IMG_0260.jpg?ex=6550f892&is=653e8392&hm=1b9ba1880c4a665705a7908585f3cdddbfda9683f832cf94d51e1e0ef7b326d9&=&width=447&height=662",
      author: "Kao Gia Bỉm",
      timestamp: new Date(),
      text: "Cho em hỏi mình làm cả 2 ca thì được tổng là 1 ngày CTXH đúng không ạ?",
    }
];
